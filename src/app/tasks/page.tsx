...
const Page = async () => {
  ...
  return (
    <div className="flex flex-col bg-white w-screen h-screen text-black items-center p-10">
      <div className="flex flex-row w-full max-w-1/2 justify-between items-center pb-2">
        <p>Tasks: {tasks.length}</p>
        <p>Completed: {completed.length} of {tasks.length}</p>
        <Link href="/tasks/new" className="px-4 py-2 bg-blue-600 rounded-md text-white cursor-pointer">Add New</Link>
      </div>
      <Tasks tasks={tasks} />
    </div>
  )
